...
for pair in ["EUR/USD", "GBP/USD"]:
    sep_res = d.get("results", {}).get(pair, {})
    sep_bal = sep_res.get("final_balance", "?")
    sep_roi = sep_res.get("roi_pct", "?")
    print(f"  {pair} alone : ${sep_bal}  ({sep_roi:+.2f}%)" if isinstance(sep_roi, float) else f"  {pair} alone : ${sep_bal}")
